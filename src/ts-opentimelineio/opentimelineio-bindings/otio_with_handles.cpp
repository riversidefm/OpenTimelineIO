...
// Basic OTIO includes (avoiding problematic ones)
...
std::string get_version() {
    return "OTIO Complete Bindings 2.0";
}
...
EMSCRIPTEN_BINDINGS(otio_with_handles) {
    ...
    function("track_kind", +[](size_t ptr) -> std::string {
        if (ptr == 0) return "";
        Track* obj = reinterpret_cast<Track*>(ptr);
        return obj ? obj->kind() : "";
    });
    ...
    function("clip_source_range", +[](size_t ptr) -> TimeRange {
        if (ptr == 0) return TimeRange();
        Clip* obj = reinterpret_cast<Clip*>(ptr);
        if (!obj) return TimeRange();
        auto sr = obj->source_range();
        return sr.has_value() ? sr.value() : TimeRange();
    });
    ...
    function("external_reference_available_range", +[](size_t ptr) -> TimeRange {
        if (ptr == 0) return TimeRange();
        ExternalReference* obj = reinterpret_cast<ExternalReference*>(ptr);
        if (!obj) return TimeRange();
        auto ar = obj->available_range();
        return ar.has_value() ? ar.value() : TimeRange();
    });
    ...
    function("composition_child_at_index", +[](size_t ptr, int index) -> size_t {
        if (ptr == 0) return 0;
        Composition* obj = reinterpret_cast<Composition*>(ptr);
        if (!obj) return 0;
        const auto& children = obj->children();
        if (index < 0 || index >= children.size()) return 0;
        return reinterpret_cast<size_t>(children[index]);
    });
    ...
    function("get_object_schema_name", +[](size_t ptr) -> std::string {
        if (ptr == 0) return "";
